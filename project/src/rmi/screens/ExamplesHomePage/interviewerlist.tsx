...
const InterviewerList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInterviewers = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/interviewers');
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {interviewers.map((interviewer) => (
            <div
              key={interviewer._id}
              className={`p-4 rounded-2xl shadow-md text-white cursor-pointer bg-cover bg-center relative`}
              style={{
                backgroundImage: "url('https://source.unsplash.com/random/400x300/?office,workspace')"
              }}
              onClick={() => {
            ...
            >
              <div className="bg-black bg-opacity-60 p-4 rounded-xl">
                ...
                <p className="text-sm">{interviewer.position} at {interviewer.company}</p>
                <div className={`mt-2 inline-block px-3 py-1 rounded-full text-white ${getRatingColor(interviewer.rating)}`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl p-6 shadow-lg w-96 text-center">
            ...
            <button
              ...
              className="w-full bg-red-500 text-white py-2 rounded-lg mb-2 hover:bg-red-600"
            >
              Not Cleared
            </button>
            <button
              onClick={() => handleInterviewStatus('waiting')}
              className="w-full bg-yellow-500 text-white py-2 rounded-lg mb-2 hover:bg-yellow-600"
            >
              Waiting for Result
            </button>
            <button
